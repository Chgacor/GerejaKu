                ...
                @forelse($articles as $article)
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        ...
                        <td class="py-3 px-6 text-left">
                            {{-- PENGAMAN TAMBAHAN: Gunakan optional() untuk mencegah error jika relasi rusak --}}
                            <span class="bg-gray-200 text-gray-700 py-1 px-3 rounded-full text-xs">{{ optional($article->commission)->name }}</span>
                        </td>
                        <td class="py-3 px-6 text-left">{{ $article->category }}</td>
                        <td class="py-3 px-6 text-left">{{ $article->published_at ? \Carbon\Carbon::parse($article->published_at)->format('d M Y') : 'Draft' }}</td>
