...
@section('content')
    ...
    <div class="text-sm text-gray-600 mt-4">
        {{-- Menangani jika $jemaats kosong agar tidak error di firstItem() --}}
        @if($jemaats->count() > 0)
        ...
        <table class="min-w-full bg-white rounded-lg overflow-hidden shadow-sm">
            ...
            @forelse($jemaats as $data)
                <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-150">
                    {{-- PERBAIKAN: Menggunakan nama kolom Bahasa Inggris sesuai database baru --}}
                    <td class="py-3 px-6 text-left whitespace-nowrap font-medium">{{ $data->full_name }}</td>
                    <td class="py-3 px-6 text-left">{{ $data->gender }}</td>
                    <td class="py-3 px-6 text-left">{{ $data->phone_number }}</td>
                    <td class="py-3 px-6 text-left">{{ Str::limit($data->address, 35) }}</td>
                    <td class="py-3 px-6 text-center">
                        <div class="flex item-center justify-center space-x-2">
                            {{-- Tombol View --}}
                            <a href="{{ route('admin.jemaat.show', $data->id) }}"
                               class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 text-blue-500 hover:bg-blue-500 hover:text-white transition transform hover:scale-110"
                               title="Lihat Detail">
                            ...
                            <a href="{{ route('admin.jemaat.edit', $data->id) }}"
                               class="w-8 h-8 flex items-center justify-center rounded-full bg-yellow-100 text-yellow-600 hover:bg-yellow-500 hover:text-white transition transform hover:scale-110"
                               title="Edit Data">
                            ...
                            <form action="{{ route('admin.jemaat.destroy', $data->id) }}" method="POST"
                                  onsubmit="return confirm('Apakah Anda yakin ingin menghapus data {{ $data->full_name }}?');"
                                  class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="w-8 h-8 flex items-center justify-center rounded-full bg-red-100 text-red-500 hover:bg-red-500 hover:text-white transition transform hover:scale-110"
                                        title="Hapus Data">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
            ...
            @empty
                <tr>
                    <td colspan="5" class="py-4 text-center text-gray-500">
                        Belum ada data jemaat. Silakan tambah data baru.
                    </td>
                </tr>
            @endforelse
            </tbody>
